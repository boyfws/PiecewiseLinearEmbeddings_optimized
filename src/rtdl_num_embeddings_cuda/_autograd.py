...
def _setup_forward_context(
    ctx: Any,
    inputs: tuple[Tensor, Tensor, Tensor, Tensor, torch.dtype],
    output: Tensor,
) -> None:
    del output
    x, bin_edges, n_bins, weight, _output_dtype = inputs
...
) -> tuple[Tensor | None, None, None, Tensor | None, None]:
    if grad_output is None:
        return None, None, None, None, None

    x, bin_edges, n_bins, weight = ctx.saved_tensors
    compute_grad_x = bool(ctx.needs_input_grad[0])
    compute_grad_weight = bool(ctx.needs_input_grad[3])
