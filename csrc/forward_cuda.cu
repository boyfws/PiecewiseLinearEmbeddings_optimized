...
template <typename output_t, int kTileD>
__global__ void ple_forward_shared_kernel(
    ...
    for (
        ...
        // All groups execute the same iteration count, which keeps the warp
        // shuffle masks valid for kTileD=16 and kTileD=32.
        for (
            ...
            if (
                ...
                const float quantized_weight = (
                    quantize_to_float<output_t>(weight[weight_index])
                );
...
void launch_ple_forward(
    ...
    TORCH_CHECK(
        ...
        "The launch tile count overflows int64"
    );
    ...
    if (use_shared_path) {
        if (tile_d == 16) {
            C10_CUDA_CHECK(
                cudaFuncSetAttribute(
                    ple_forward_shared_kernel<output_t, 16>,
            ...
            C10_CUDA_CHECK(
                cudaFuncSetAttribute(
                    ple_forward_shared_kernel<output_t, 32>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    static_cast<int>(shared_bytes)
                )
            );
        ...
        ple_forward_generic_kernel<output_t><<<
            generic_blocks,
            kBlockThreads,
            0,
            stream
...
at::Tensor ple_forward_cuda(
    ...
    TORCH_CHECK(
        ...
        "x, bin_edges, n_bins, and weight must be on the same CUDA device"
    );
    ...
    if (output_dtype == at::kFloat) {
        rtdl_num_embeddings_cuda::launch_ple_forward<float>(
            x,
            bin_edges,
            n_bins,
            weight,
        ...
        rtdl_num_embeddings_cuda::launch_ple_forward<c10::Half>(
            x,
            bin_edges,
            n_bins,
            weight,
        ...
        rtdl_num_embeddings_cuda::launch_ple_forward<c10::BFloat16>(
            x,
            bin_edges,
            n_bins,
            weight,
